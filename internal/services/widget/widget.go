...
type WidgetUpdater interface {
	...
	UpdateConfig(id int, config string) error
}
...
func (service *Service) UpdateConfig(ctx context.Context, id int, config string) error {
	err := service.widgetUpdater.UpdateConfig(id, config)
	if err != nil {
		return nil
	}
